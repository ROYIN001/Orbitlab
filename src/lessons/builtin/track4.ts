...
export const TRACK4: readonly unknown[] = [
  ...
  },
  {
    ...
    brief: {
      en: 'Test the pitch autopilot in flight: at about T+90 s, command a 2° pitch step held for 8 s from the attitude-loop inspector\'s flight test, and read its overshoot. With the default gains it overshoots by nearly 40 %. Retune the pitch–yaw gains so that the step overshoots by no more than 35 %, fly the test again, and type in the overshoot you measured.',
      ru: 'Испытайте автомат стабилизации по тангажу в полёте: около T+90 с подайте из раздела лётных испытаний инспектора контура ступенчатую команду 2° по тангажу длительностью 8 с и определите перерегулирование. Со штатными коэффициентами оно почти 40 %. Перенастройте коэффициенты тангажа–рыскания так, чтобы перерегулирование было не больше 35 %, повторите испытание и введите измеренное перерегулирование.',
      th: 'ทดสอบระบบรักษาท่าทางช่องพิตช์ระหว่างบิน ที่ประมาณ T+90 วินาที สั่งคำสั่งขั้นพิตช์ 2° ค้างไว้ 8 วินาที จากส่วนทดสอบการบินของตัวตรวจลูป แล้วอ่านโอเวอร์ชูต ด้วยเกนเริ่มต้นจะเกินเกือบ 40% จงปรับเกนพิตช์–ยอว์ใหม่ให้โอเวอร์ชูตไม่เกิน 35% บินทดสอบอีกครั้ง แล้วพิมพ์ค่าโอเวอร์ชูตที่วัดได้',
    },
    debrief: {
      en: 'A step in flight shows what the margins predict: little damping, a large overshoot. Raising the rate gain K_ω damps the answer. The flown response also carries what the linear model leaves out — rate and gimbal limits, the air changing as the rocket climbs — which is why a flight test is flown at all.',
      ru: 'Ступенчатое воздействие в полёте показывает то, что предсказывают запасы: мало демпфирования — большое перерегулирование. Увеличение K_ω демпфирует реакцию. В реальном полёте на реакцию влияет и то, чего нет в линейной модели, — ограничения скорости и отклонения сопла, изменение атмосферы по мере подъёма; именно поэтому лётные испытания и проводят.',
      th: 'การทดสอบขั้นระหว่างบินแสดงสิ่งที่มาร์จินทำนายไว้ ความหน่วงน้อยให้โอเวอร์ชูตมาก การเพิ่มเกนอัตรา K_ω ช่วยหน่วงการตอบสนอง การตอบสนองจริงยังรวมสิ่งที่แบบจำลองเชิงเส้นไม่มี เช่น ขีดจำกัดอัตราและมุมหัวฉีด และอากาศที่เปลี่ยนไปเมื่อจรวดไต่ระดับ จึงต้องมีการทดสอบการบินจริง',
    },
    mission: falcon9(),
    locked: [...LOCKED, 'setup.boosterRecovery'],
    endEvent: 'evt.meco',
    criteria: [
      { id: 'overshoot', kind: 'measure', measure: 'step.overshoot', max: 35 },
      { id: 'flying', kind: 'outcome', is: 'survived' },
      {
        id: 'overshoot-read', kind: 'answer', measure: 'step.overshoot', tol: 5, unit: '%',
    ...
    hints: [
      { en: 'Attitude-loop inspector → flight test: pitch, a step of 2°, held 8 s. Press it while the rocket climbs, at about T+90 s; the last step flown before MECO is graded.', ru: 'Инспектор контура стабилизации → лётные испытания: тангаж, ступенька 2°, 8 с. Нажмите при подъёме, около T+90 с; оценивается последнее воздействие до MECO.', th: 'ตัวตรวจลูปควบคุมท่าทาง → ทดสอบการบิน: พิตช์ ขั้น 2° ค้าง 8 วินาที กดขณะจรวดไต่ระดับที่ประมาณ T+90 วินาที ระบบจะให้คะแนนการทดสอบขั้นครั้งสุดท้ายก่อน MECO' },
      ...
      { en: 'Raise the rate gain K_ω: at 6 the overshoot falls to about 30 %.', ru: 'Увеличьте K_ω: при 6 перерегулирование снижается примерно до 30 %.', th: 'เพิ่มเกนอัตรา K_ω ที่ 6 โอเวอร์ชูตลดลงเหลือราว 30%' },
    ],
  },
  {
