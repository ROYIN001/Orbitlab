  ...
  update(sim: Simulation | null, warp: number): void {
    ...
    this.left.innerHTML = `
      ...
      <div class="hud-grid">
        ...
        <span class="k">${t('hud.thrust')}</span><span class="v">${(s.thrust / 1000).toFixed(0)} kN</span>
        <span class="k">${t('hud.mass')}</span><span class="v">${(s.mass / 1000).toFixed(1)} t</span>
        ...
        <span class="k">${t('hud.warp')}</span><span class="v">${warp}×</span>
      </div>`;
    ...
    this.right.innerHTML = `
      <div class="hud-grid">
        ...
        <span class="k">${t('hud.speed')}</span><span class="v">${s.speed.toFixed(0)} m/s</span>
        <span class="k">${t('hud.airspeed')}</span><span class="v">${s.airspeed.toFixed(0)} m/s</span>
        <span class="k">${t('hud.vertical')}</span><span class="v">${s.vz.toFixed(0)} m/s</span>
        <span class="k">${t('hud.q')}</span><span class="v">${(s.q / 1000).toFixed(1)} kPa</span>
        ...
        <span class="k">${t('hud.period')}</span><span class="v">${isFinite(el.period) ? (el.period / 60).toFixed(1) + ' min' : '—'}</span>
        <span class="k">${t('hud.dv')}</span><span class="v">${(s.payloadSeparated ? sim.vehicle.spacecraftDeltaV() : sim.vehicle.deltaVRemaining()).toFixed(0)} m/s</span>
    ...
    while (this.shownEvents < sim.events.length) {
      ...
      d.textContent = `${fmtTime(e.t)}  ${t(e.key, e.params)}`;
