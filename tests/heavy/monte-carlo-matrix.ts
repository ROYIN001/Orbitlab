...
import {
  drawLayout, flyMonteCarloRun, missionTargetsOf, summarizeMonteCarlo, type GuidanceLaw, type MeasurePoint, type MonteCarloConfig, type MonteCarloRun,
...
export function monteCarloSet(c: MonteCarloCase): void {
  it(`${c.vehicle} ${c.law}${c.navigation ? ' with navigation' : ''}: ${c.runs} dispersed runs insert within the bands`, () => {
    ...
    console.log(`MC ${c.vehicle} ${c.law}${c.navigation ? '+nav' : ''} ${ref.orbit}: ${law.inserted}/${law.runs} in orbit, ${law.onTarget} on target, lost ${law.lost} `
      + `${JSON.stringify(law.reasons)}; ${at('final')} | ${at('cutoff')}; max q·α ${f(Math.max(...runs.map((r) => r.maxQAlpha)), 0)} kPa·°; `
      + `${f(runs.reduce((a, r) => a + r.ms, 0) / runs.length / 1000, 1)} s/run`);
    const s = law.points.final.stats;
